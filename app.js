...
const LANG = {
  sk: {
    ...
    btnImportUrl:'Import z URL', importUrlTitle:'🌐 Importovať recept z URL', importUrlHint:'Vlož odkaz na recept (varecha.pravda.sk) – aplikácia ho stiahne a predvyplní formulár.', importUrlDo:'📥 Importovať', importUrlOk:'✅ Recept načítaný! Skontroluj a ulož.', importUrlFail:'❌ Nepodarilo sa načítať recept. Skús iný odkaz.', importUrlBad:'❌ Nenašiel sa žiadny recept na tejto URL. Skús iný odkaz.',
...
// NOTE: For production, move the API key to a backend proxy.
// This is a client-side PWA; the key is visible in source.
const PEXELS_KEY = 'REMOVED_PEXELS_KEY';
...
function renderBoard() {
  ...
  boardCards.forEach(function(card) {
    var name = card.authorName || (lang === 'en' ? 'Someone' : 'Niekto');
...
function openAddBoardCard(editId) {
  ...
  var textVal = card ? (card.text || '') : '';
  var imageVal = card ? (card.image || '') : '';
  ...
  div.innerHTML = '<div class="modal" style="max-width:480px;">' +
    '<button class="modal-close" onclick="document.getElementById(\'board-form-modal\').remove()">✕</button>' +
    '<h2>' + title + '</h2>' +
    ...
    '<textarea id="board-form-text" rows="3" style="width:100%;padding:.55rem;border:1px solid var(--border);border-radius:8px;font-size:.85rem;background:var(--input-bg);color:var(--text);resize:vertical;">' + esc(textVal) + '</textarea></div>' +
    ...
    '<input id="board-form-image" value="' + escAttr(imageVal) + '" placeholder="https://..." style="width:100%;padding:.55rem;border:1px solid var(--border);border-radius:8px;font-size:.85rem;background:var(--input-bg);color:var(--text);"></div>' +
    ...
    ['var(--surface)','rgba(220,38,38,.15)','rgba(16,185,129,.15)','rgba(59,130,246,.15)','rgba(245,158,11,.15)','rgba(139,92,246,.15)'].map(function(c) {
      return '<span class="board-color-dot' + ((card ? card.color : 'var(--surface)') === c ? ' active' : '') + '" data-color="' + c + '" style="background:' + c + ';border:2px solid ' + (c === 'var(--surface)' ? 'var(--border)' : c.replace('.15)','.3)')) + ';" onclick="this.parentElement.querySelectorAll(\'.board-color-dot\').forEach(function(d){d.classList.remove(\'active\')});this.classList.add(\'active\');"></span>';
    }).join('') + '</div>' +
  ...
  document.body.appendChild(div);
  document.getElementById('board-form-text').focus();
}

function saveBoardCard(editId) {
  ...
  if (editId) {
    ...
    boardCards.push({
      ...
      pinned: pinned,
      reactions: {},
      authorName: authUser ? (authUser.displayName || authUser.email || '👤') : (lang==='en'?'Guest':'Hosť'),
      createdAt: Date.now(),
      updatedAt: Date.now()
    });
  }
  saveBoardCards();
  syncBoardToFirebase();
  document.getElementById('board-form-modal')?.remove();
  renderBoard();
}

function deleteBoardCard(id) {
  showConfirmModal(
    lang==='en'?'Delete this card?':'Vymazať túto kartu?', '🗑️',
    lang==='en'?'Delete':'Vymazať',
    function() {
      loadBoardCards();
...
function togglePinBoardCard(id) {
  loadBoardCards();
...
function editBoardCardColor(id) {
  ...
  var nextIdx = (currentIdx + 1) % colors.length;
  card.color = colors[nextIdx];
  saveBoardCards();
  syncBoardToFirebase();
  renderBoard();
}

function toggleBoardReaction(cardId, emoji) {
  loadBoardCards();
  ...
  if (idx > -1) {
    card.reactions[emoji].splice(idx, 1);
    if (!card.reactions[emoji].length) delete card.reactions[emoji];
  } else {
    card.reactions[emoji].push(deviceId);
  }
  saveBoardCards();
  syncBoardToFirebase();
  renderBoard();
...
function openReactionPicker(event, cardId) {
  ...
  picker.innerHTML = BOARD_REACTIONS.map(function(r) {
    return '<button style="font-size:1.4rem;width:40px;height:40px;border:none;background:none;cursor:pointer;border-radius:8px;transition:background .15s;" onmouseover="this.style.background=\'var(--hover-bg)\'" onmouseout="this.style.background=\'none\'" onclick="toggleBoardReaction(\'' + cardId + '\',\'' + r + '\');this.parentElement.remove()">' + r + '</button>';
  }).join('');
  document.body.appendChild(picker);
  setTimeout(function() { document.addEventListener('click', function _close() { var p = document.getElementById('board-reaction-picker'); if (p) p.remove(); document.removeEventListener('click', _close); }, { once: true }); }, 10);
}

function toggleBoardCardMenu(el) {
...
function syncBoardToFirebase() {
  if (!familyDbRef) return;
  try {
    familyDbRef.child('boardCards').set(boardCards, function(err) {
      if (err) console.error('Board sync error:', err.message);
    });
  } catch(e) { /* silent */ }
}
...
function initBoardFirebaseSync() {
  ...
  ref.on('value', function(snap) {
    ...
    // Merge: prefer remote, but keep local-only cards
    var map = {};
    ...
    saveBoardCards();
    // Re-render if board tab is active
    if (document.body.dataset.tab === 'board') renderBoard();
  });
  firebaseListeners.push(ref);
}

// Helper: time ago in slovak/english
function timeAgo(ts) {
  var diff = Date.now() - ts;
  var mins = Math.floor(diff / 60000);
  if (mins < 1) return t('práve teraz','just now');
  if (mins < 60) return mins + ' ' + (mins === 1 ? (lang==='en'?'min ago':'minútou') : (lang==='en'?'mins ago':'minútami'));
  var hours = Math.floor(mins / 60);
  if (hours < 24) return hours + ' ' + (hours === 1 ? (lang==='en'?'hour ago':'hodinou') : (lang==='en'?'hours ago':'hodinami'));
